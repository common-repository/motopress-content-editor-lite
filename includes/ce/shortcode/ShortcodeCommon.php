...
//			wp_enqueue_style('mpce-theme');
        ...
        if (!empty($bg_video_youtube)) {
            if (preg_match('/^(?:http(?:s)?:\/\/)?(?:www\.)?(?:youtu\.be\/|youtube\.com\/(?:(?:watch)?\?(?:.*&)?v(?:i)?=|(?:embed|v|vi|user)\/))([^\?&\"\'>]+)/', $bg_video_youtube, $idVideo)) {
	            ...
	            if ( mpceIsEditorScene() ) {
                    ...
                    $dataMute = ($bg_video_youtube_mute == 'true') ? '&mute=1' : '';
                    $videoHTML .= '<iframe class="mp-youtube-video" src="https://www.youtube.com/embed/' . $idVideo[1] . '?controls=0&rel=0&showinfo=0&autoplay=1' . $loop . '&disablekb=1&showsearch=0&iv_load_policy=3&enablejsapi=1&vq=hd720' . $playlist . $dataMute . '"></iframe>';
                }
    ...
    function mp_gmap($attrs, $content = null, $tag) {
		...
		$googleApiKey = sanitize_text_field(get_option('motopress-ce-google-geocode-api-key', ''));
		...
		if( empty($googleApiKey ) ) {
			if ( current_user_can( 'manage_options' ) ) {
				...
				if ( self::isContentEditor() ) {
					return sprintf(
						__('Set Google Maps API Key in the %1$s plugin settings %2$s.', 'motopress-content-editor-lite'),
						'',
						''
					);
				} else {
					return sprintf(
						__('Set Google Maps API Key in the %1$s plugin settings %2$s.', 'motopress-content-editor-lite'),
						'<a href="' . admin_url('admin.php?page=motopress_options') .'" target="_blank">',
						'</a>'
					);
				}
